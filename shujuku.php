...
?>
<?php 
if($_POST['submitjizhang']){
if($_POST['password']=="admin"){ //修改密码~~~~~
...
echo "<script language=javascript>alert('登录信息错误，请登录！');window.location='denglu.php'</script>";
exit () ;  }  ?>
...
<?php
  ...
  header("location:denglu.php"); //跳转到首页
  }
?>
...
switch($_GET['action'])
{
  ...
  case "create":
    ...
    //创建这个数据库 
    $filename=$filename.".txt"; //不让你创建其他格式东东
    $f = fopen($filename,"w");
